...
# Allow override via ENV; default to localhost (or host.docker.internal in Docker)
...
def main():
    st.markdown(
        "<h1 style='text-align:center;'>📄 Resume Evaluator</h1>",
    ...
    if mode == "🧾 Simple":
        ...
        with col2:
            description = st.text_area(
                "Job Description",
                height=120,
                placeholder="e.g. Build and maintain RESTful APIs using Python & FastAPI…"
            )
        ...
        with col4:
            min_qualifications = st.text_area(
                "Minimum Qualifications",
                height=100,
                placeholder="e.g. Bachelor’s degree in Computer Science"
            )
        adv_qualifications = st.text_area(
            "Advanced Qualifications (Optional)",
            height=100,
            placeholder="e.g. Experience with distributed systems, Kubernetes…"
        )

        # build the full job_description string
        parts = []
    ...
    if st.button("🔍 Evaluate Resumes"):
        ...
        files = [
            ("resumes", (f.name, f.getvalue(), f.type))
            for f in uploaded
        ]
        ...
        for entry in results:
            ...
            if entry.get("interview_questions"):
                ...
                st.download_button(
                    ...
                    qtxt,
                    file_name=f"questions_{entry['filename']}.txt",
                    mime="text/plain"
                )
